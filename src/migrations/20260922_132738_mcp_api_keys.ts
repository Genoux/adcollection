...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
   CREATE TABLE "payload_mcp_api_keys" (
  	...
  	"platforms_find" boolean DEFAULT false,
  	"categories_find" boolean DEFAULT false,
  	"subcategories_find" boolean DEFAULT false,
  	"content_types_find" boolean DEFAULT false,
  	"payload_mcp_tool_frameio_browse" boolean DEFAULT true,
  	"payload_mcp_tool_frameio_inspect_file" boolean DEFAULT true,
  	"payload_mcp_tool_import_frameio_ad" boolean DEFAULT true,
